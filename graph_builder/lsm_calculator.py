...
class BaseLSMCalculator(ABC):
    ...
    def _sum_xi_squared_yi(self) -> float:
        return sum(
            [
                (point_x**2) * point_y
                for point_x, point_y in zip(self._x_points, self._y_points)
    ...
    @abstractmethod
    def find_lsm_points(self) -> PointsTuple:
        raise NotImplementedError
...
class LinearCalculator(BaseLSMCalculator):
    ...
    def calculate(self) -> Tuple[float, ...]:
        left_matrix_half = np.array(
            [
                [self._point_num, self._sum_xi()],
                [self._sum_xi(), self._sum_xi_squared()],
            ]
        )

        right_matrix_half = np.array([self._sum_yi(), self._sum_xi_yi()])
...
class ParabolicCalculator(BaseLSMCalculator):
    ...
    def calculate(self) -> Tuple[float, ...]:
        left_matrix_half = np.array(
            [
                [self._point_num, self._sum_xi(), self._sum_xi_squared()],
                [self._sum_xi(), self._sum_xi_squared(), self._sum_xi_cubed()],
                [
                    self._sum_xi_squared(),
                    self._sum_xi_cubed(),
                    self._sum_xi_quadrupled(),
                ],
            ]
        )

        right_matrix_half = np.array(
            [self._sum_yi(), self._sum_xi_yi(), self._sum_xi_squared_yi()]
        )
